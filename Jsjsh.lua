...
local PresetColor = Color3.fromRGB(102, 0, 204)
...
MinimizeButton.Position = UDim2.new(0.5, -26.5, 0.5, -25)
...
MinimizeUIStroke.Color = Color3.fromRGB(255, 0, 0)
MinimizeUIStroke.Thickness = 2

local MinimizeButtonVisible = true

local function MinimizeButtonFunction(options)
    if options.Image then
        MinimizeButton.Image = options.Image
    end
    
    if options.Size then
        MinimizeButton.Size = UDim2.new(0, options.Size[1], 0, options.Size[2])
        MinimizeButton.Position = UDim2.new(0.5, -options.Size[1]/2, 0.5, -options.Size[2]/2)
    end
    
    if options.Color then
        MinimizeButton.BackgroundColor3 = options.Color
    end
    
    if options.Corner and MinimizeUICorner then
        MinimizeUICorner.CornerRadius = UDim.new(0, 8)
    end
    
    if options.Stroke and MinimizeUIStroke then
        MinimizeUIStroke.Thickness = 2
        if options.StrokeColor then
            MinimizeUIStroke.Color = options.StrokeColor
        end
    end
    
    MinimizeButton.MouseButton1Click:Connect(function()
        if Frame.Visible then
            Frame:TweenSize(UDim2.new(0, 0, 0, 0), "Out", "Quad", 0.3, true, function()
                Frame.Visible = false
                MinimizeButtonVisible = true
            end)
        else
            Frame.Visible = true
            Frame:TweenSize(UDim2.new(0, 500, 0, 270), "Out", "Quad", 0.3, true)
